...
exports.syncTraceCalendar = void 0;
...
// Config
const CLIENT_ID = ((_a = functions.config().google) === null || _a === void 0 ? void 0 : _a.client_id) || "MISSING_CLIENT_ID";
const CLIENT_SECRET = ((_b = functions.config().google) === null || _b === void 0 ? void 0 : _b.client_secret) || "MISSING_CLIENT_SECRET";
// You should set these using: firebase functions:config:set google.client_id="..." google.client_secret="..."
exports.syncTraceCalendar = functions.pubsub.schedule("every 60 minutes").onRun(async (context) => {
    var _a;
    ...
    for (const docSnap of settingsSnaps.docs) {
        if (docSnap.id !== "calendar")
            continue; // Ensure we are looking at the calendar settings doc
...
async function syncUserCalendar(uid, settings) {
    // 1. Get Credentials
    ...
    const classroom = googleapis_1.google.classroom({ version: "v1", auth: oauth2Client });
    ...
    if (settings.syncAssignments) {
        ...
        for (const course of courses) {
            ...
            for (const work of works) {
                ...
                if (mapDoc.exists) {
                    // Update
                    const { calendarEventId } = mapDoc.data();
                    try {
