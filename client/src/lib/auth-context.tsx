...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const enhancedUser = useMemo(() => {
    ...
    return new Proxy(user, {
      get(target, prop) {
        if (prop === "photoURL") return customPhotoURL;
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      // 🔥 FIX: Persist Firebase session (prevents 7–8h logout)
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        accessToken,
        signInWithGoogle,
        requestGmailPermissions: async () => true,
        requestCalendarPermissions: async () => true,
